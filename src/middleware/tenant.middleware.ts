...
/**
 * Platform-agnostic request interface supporting Express and Fastify
 */
interface PlatformRequest {
  method: string;
  url: string;
  path?: string; // Express
  routerPath?: string; // Fastify
  headers: Record<string, string | string[] | undefined>;
  hostname?: string;
  query?: Record<string, unknown>;
  cookies?: Record<string, string>;
  raw?: { url?: string }; // Fastify raw request
}

/**
 * Platform-agnostic next function
 */
type PlatformNextFunction = (error?: unknown) => void;
import {
  DEFAULT_CACHE_MAX,
  DEFAULT_CACHE_TTL,
...
export class TenantMiddleware implements NestMiddleware {
  ...
  async use(req: PlatformRequest, _res: unknown, next: PlatformNextFunction): Promise<void> {
  ...
  /**
   * Get request path (works with both Express and Fastify)
   */
  private getRequestPath(req: PlatformRequest): string {
    // Express uses req.path, Fastify uses req.url or req.routerPath
    if (req.path) {
      return req.path;
    }
  ...
  /**
   * Get request hostname (works with both Express and Fastify)
   */
  private getRequestHostname(req: PlatformRequest): string {
    // Both Express and Fastify support req.hostname
    if (req.hostname) {
      return req.hostname;
    }
   ...
   */
  private async extractTenantId(req: PlatformRequest): Promise<null | string> {
    const strategy = this.options.extractionStrategy ?? 'header';
   ...
   */
  private extractFromHeader(req: PlatformRequest): null | string {
   ...
   */
  private extractFromSubdomain(req: PlatformRequest): null | string {
    const host = this.getRequestHostname(req);
   ...
   */
  private extractFromPath(req: PlatformRequest): null | string {
   ...
   */
  private extractFromQuery(req: PlatformRequest): null | string {
   ...
   */
  private extractFromCookie(req: PlatformRequest): null | string {
   ...
   */
  private extractFromJwt(req: PlatformRequest): null | string {
   ...
   */
  private async extractFromBearer(req: PlatformRequest): Promise<null | string> {
   ...
   */
  private async extractCustom(req: PlatformRequest): Promise<null | string> {
